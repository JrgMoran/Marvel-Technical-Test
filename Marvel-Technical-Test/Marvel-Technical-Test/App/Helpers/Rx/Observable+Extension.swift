...
func <-> <T: Equatable>(property: BehaviorRelay<T>, variable: BehaviorRelay<T>) -> Disposable {
    let bindToUIDisposable = variable.filter({
        $0 != property.value
        
    }).asObservable().bind(to: property)
    
    let bindToVariable = property.filter({
        $0 != variable.value
        
    }).asObservable().bind(to: variable)
